...
void main()
{
	...
	for (i = 0; i < n; i++) {
		...
		// поиск суммы ряда
		sum = sl = x[i]*(3-x[i])/(3*2*1);  // sl=pow(x[i],2); sum=sl;  // первое слагаемое // pow - воведение в степень
		k = 1;
		while ((fabs(sl) >= e) && (k != 500)) {  // fabs - модуль
			sl *= - ((2*k*pow(x[i],2)) + 3*pow(x[i], 2) - pow(x[i], 3))/((4*pow(k,2) + 10*k + 6)*(2*k + 1 - x[i]));
			sum += sl;
			k++;
		};

		f = ((1 - cos(x[i]) + sin(x[i]))/ x[i]) - 1;
